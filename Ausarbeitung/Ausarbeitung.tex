...
Grundsätzlich gibt es zwei verschiedene Kategorien von Lösungsansätzen: \textit{exakte Algorithmen} und \textit{Heuristiken}. Unter der Voraussetzung einer beliebig langen Laufzeit, finden \textit{exakte Algorithmen} eine Optimallösung. \textit{Heuristiken} dagegen garantieren nicht, dass eine optimale Lösung gefunden wird, sondern versuchen innerhalb eines Zeitrahmens eine zulässige Lösung zu finden. Unter mithilfe von Entscheidungsregeln wird sich dabei möglichst nah an die optimale Lösung angenähert.
...
\par Das ist die Ursache dafür, dass in der Praxis Heuristiken verwendet werden, obwohl sie nicht unbedingt die optimale Lösung liefern. Aufgrund der Tatsache, dass die verwendeten Daten schon Fehler enthalten oder nicht genau ermittelt werden können, sind suboptimale Lösungen, die möglichst nahe dem Optimum liegen, häufig ausreichend. Zum Beispiel ist eine Städtetour durch Europa mit einem Fahrzeug von vielen Faktoren abhängig wie: Fahrzeugtyp, Wetter oder Verkehr. In solchen Anwendungen ist die Forderung nach Optimalität überzogen.

\subsubsection{TSP-Herausforderungen}
Das TSP ist eines der am intensivsten erforschten Probleme in der Computermathematik. Die Seite \textit{http://www.math.uwaterloo.ca/tsp/index.html} widmet sich der Geschichte, den Anwendungen und der aktuellen Forschung dieser Herausforderung. Dort werden auch sämtliche Rekorde rund um das Thema TSP festgehalten.
\par Als Beispiel am 20. April 2001 verkündeten David Applegate, Robert Bixby, Vašek Chvátal und William Cook die optimale Lösung eines TSP-Problems durch 15.112 Städte in Deutschland. Dies war die größte TSP-Herausforderung, die zu der Zeit gelöst worden war, und übertraf die 13.509 Städte Tour durch die Vereinigten Staaten, die 1998 gelöst wurde. Die optimale Tour war $66.000km$ lang. Die Berechnung wurde in einem Netzwerk von 110 Prozessoren an der Rice University und der Princeton University durchgeführt. Die gesamte Computerzeit, die bei der Berechnung verwendet wurde, betrug 22,6 Jahre.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{detsp.jpg}
	\caption{Lösung eines TSP-Problems durch 15.112 Städte in Deutschland}
	\label{detsp}
\end{figure}

Die aktuelle (Stand 09.05.18) Herausforderung ist es eine optimale Tour durch 100,000 Städte zu finden. Das Preisgeld für dieses Problem beträgt 1000\$.
