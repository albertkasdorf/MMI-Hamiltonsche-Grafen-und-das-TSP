...
Das \textit{Traveling Salesman Problem} oder \textit{Problem des Handlungsreisenden}, wie es auf deutsch heißt, ist ein kombinatorisches Optimierungsproblem und ein Anwendungsfall der Hamilton Kreise. Die klassische Herausforderung besteht, darin eine optimale Reihenfolge für den Besuch einer vorgegebenen Menge von Orten so zu planen, dass die zurückgelegte Strecke des Handlungsreisenden möglichst kostenminimal, kein Ort soll doppelt besucht werden und die erste Station gleich der letzten Station ist.

\subsection{Definition}
Das allgemeine \textit{Traveling Salesman Problem} ist wie folgt definiert:
\begin{mydef}
	Travelling-Sales-Problem (TSP): Gegeben sei ein vollständiger Graph K\textsubscript{n} mit positiven Gewichten \textit{c(e)} auf den einzelnen Kanten  \textit{e}. Gesucht ist ein Hamilton-Kreis \textit{C} in K\textsubscript{n} mit minimalem Gewicht c(C), wobei gilt
	\[c(C) =  \sum_{e \in C}  c(e) \]
\end{mydef}

In der Regel wird beim TSP von einem vollständigen Graphen ausgegangen, ansonsten ist im vorliegenden Problem eine Rundreise nicht möglich. Beim allgemeinen \textit{asymmetrischen TSP} können die Kanten in Hin- und Rückrichtung unterschiedliche Längen haben, so dass dieses Problem mit Hilfe eines gerichteten Graphen modelliert werden muss. Falls für alle Knotenpaare $c(u,w) = c(w,u)$ gilt, dann spricht man von einem \textit{symmetrischen TSP}, dieser kann durch ungerichteten Graphen repräsentiert werden.
...
\begin{figure}[h]
	...
	\begin{subfigure}{0.3\linewidth}
		\centering
		\begin{tikzpicture}
		\node (2) at (1,0) [node_normal] {2};
		\node (4) at (0,1) [node_normal] {4};
		\node (1) at (1,2) [node_normal] {1};
		\node (3) at (2,1) [node_normal] {3};
	...
	\begin{subfigure}{0.3\linewidth}
		\centering
		\begin{tikzpicture}
		\node (4) at (1,0) [node_normal] {4};
		\node (2) at (0,1) [node_normal] {2};
		\node (1) at (1,2) [node_normal] {1};
		\node (3) at (2,1) [node_normal] {3};
