...
\begin{figure}
	...
	\caption{Links: Ein Graph der die Bekanntschaft zwischen den Hochzeitsgästen modelliert. Rechts: Ein Kreis in dem Bekanntschaftsgraphen jeden Gast zwei im bekannte Gäste zuweist.}
...
Wie es bereits aus dem einleitenden Beispiel zu erkennen ist, wird in einem Graphen ein Kreis gesucht, der alle Knoten so miteinander verbindet, dass außer dem Start- und Endknoten kein Knoten doppelt vorkommt. Ein Graph der diese Eigenschaft erfüllt heißt hamiltonsch oder Hamilton-Graph und der Kreis heißt Hamilton-Kreis.
...
\begin{figure}
	...
	\caption{Der Graph (a) ist sowohl eulersch als auch hamiltonsch, (b) ist nur hamiltonsch, (c) ist nur eulersch und (d) ist weder noch.}
	\label{fig:combination_of_euler_and_hamilton}
\end{figure}

% TODO: Was kommt in den nächsten Abschnitten?
...
Im Jahre 1952 hat der britischer Physiker Dirac ein Kriterium aufgestellt mit der die Existenz eines Hamilton-Kreises in einem Graphen nachgewiesen werden kann, siehe Satz~\autoref{satz_dirac}.

% Was sagt dieser Satz aus? Wenn man viele Kanten hat, dann ist es wahrscheinlicher einen hamilton kreis zu finden.
\begin{mysat}\label{satz_dirac}
...
Es sei zu erwähnen, dass der Satz von Dirac eine hinreichende jedoch keine notwendige Bedingung für die Existenz eines Hamilton-Kreises ist. Es gibt Graphen die das Kriterium von Dirac nicht erfüllen und trotzdem einen Hamilton-Kreis besitzen, siehe \autoref{fig:hamilton_without_dirac_and_ore}.
...
\begin{figure}
	...
	\label{fig:hamilton_without_dirac_and_ore}
\end{figure}
...
\begin{mysat}\label{decay_to_connected_components}
...
Den Satz~\autoref{decay_to_connected_components} kann man sich dabei sehr vereinfacht wie folgt vorstellen. Man nehme einen Faden und verbinde diesen zu einem Kreis. Wenn der Faden jetzt an einer zerschnitten wird, entsteht maximal ein Fadenstück. Mit jedem weiteren Schnitt entstehen maximal zwei weitere Fadenstücke.
...
%%%%%%%%%
\subsection{Fazit}
Auf den vorherigen Abschnitten haben wir mehrere Kriterien (welche?) kennengelernt um herauszufinden ob es in einem Graphen einen Hamilton-Kreis gibt oder nicht. Nichts desto trotz existieren Graphen die die obigen Kriterien nicht erfüllen, aber über einen Kamilton-Kreis verfügen. Bisher könnte kein allgemein gültiges Kriterium gefunden für die Existenz gefunden werden und darum steht auch das Hamilton-Kreis-Problem auf der Liste der noch zu lösenden Millennium Probleme, siehe \url{http://www.claymath.org/millennium-problems}.
...
Es stellt sich nun die Frage wie gut die Lösung ist und wie weit sie von einer optimalen Lösung entfernt ist. Es lässt sich sehr anschaulich verdeutlichen, dass die Nearest Neighbor Heuristik eine beliebig schlechte Lösung für das Problem des Handlungsreisenden liefert. Gezeigt wird das an dem Grafen in \autoref{fig:nearest-neighbor-guete}.
